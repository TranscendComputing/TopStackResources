{ "Fn::Base64" : { "Fn::Join" : ["", [
  ...
  "${configuration("SERVLET_URL", "${AvailabilityZone}")} ${configuration("INTERNAL_SERVICE_HOST", "${AvailabilityZone}")}\n",
  "EOC\n",
  "cat > /etc/chef/client.rb << EOC\n",
  "log_level          :info\n",
  "log_location       STDOUT\n",
  "chef_server_url \"${configuration("CHEF_URL", "${AvailabilityZone}")}\"\n",
